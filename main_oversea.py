...
def run(start, end, num):
    ...
    for index, row in out.iloc[start: end].iterrows():
        title = row['题名']
        intro = row['简介']
    ...
    out.to_excel(r'output/海外sentiment' + num + '.xlsx', index = False) #存储文件路径
